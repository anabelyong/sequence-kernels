...
class ZeroMeanIMQHGP:
    ...
    def _kernel_matrix(self, A: list[str], B: list[str], params: IMQHGP_Params):
        key = (tuple(A), tuple(B), params.scale, params.beta, params.lag, params.alphabet_name)
        if key not in self._kernel_cache:
            self._kernel_cache[key] = jnp.asarray(imq_hamming_kernel(
                A, B,
    ...
    def _predict_with_precomputed(self, params: IMQHGP_Params,
                                  k_train_train, k_test_train, k_test_test,
                                  y_train, full_covar: bool = True):
        return kgp.noiseless_predict(
            a=TRANSFORM(params.raw_amplitude),
            s=TRANSFORM(params.raw_noise),
